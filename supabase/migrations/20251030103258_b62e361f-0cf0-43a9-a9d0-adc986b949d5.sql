-- Create enum for user plans (if not exists)
DO $$ BEGIN
...
DROP POLICY IF EXISTS "Users can view their own bilans" ON public.bilans;
DROP POLICY IF EXISTS "Users can create their own bilans" ON public.bilans;
DROP POLICY IF EXISTS "Users can update their own bilans" ON public.bilans;
DROP POLICY IF EXISTS "Users can delete their own bilans" ON public.bilans;
...
DROP POLICY IF EXISTS "Chatbots are viewable by authenticated users" ON public.chatbots;
...
DROP POLICY IF EXISTS "System can insert logs" ON public.usage_logs;
...
-- Insert default chatbots (avoid duplicates)
INSERT INTO public.chatbots (slug, titre, description, emoji, type, requires_premium, is_active)
SELECT * FROM (VALUES
  ('nomenclature-ngap', 'Aide Nomenclature NGAP', 'Trouvez le bon code acte en quelques secondes', '📖', 'nomenclature'::chatbot_type, true, true),
  ('creation-exercices', 'Création d''Exercices', 'Programmes personnalisés pour vos patients', '💪', 'exercises'::chatbot_type, true, true),
  ('posts-reseaux', 'Posts Réseaux Sociaux', 'Contenu professionnel automatisé', '📱', 'social_media'::chatbot_type, true, true),
  ('actes-hors-nomenclature', 'Actes Hors Nomenclature', 'Devis et tarifs personnalisés', '💼', 'pricing'::chatbot_type, true, true),
  ('bientot-disponible', 'Bientôt disponible', 'Nouvelles fonctions en développement', '🔜', 'coming_soon'::chatbot_type, true, false)
) AS v(slug, titre, description, emoji, type, requires_premium, is_active)
WHERE NOT EXISTS (SELECT 1 FROM public.chatbots WHERE chatbots.slug = v.slug);
...
DROP TRIGGER IF EXISTS update_bilans_updated_at ON public.bilans;
